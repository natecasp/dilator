...
function getHistory() {
  ...
  if(history !== null) {
    if (confirm("Looks like you were already working on a quote, would you like to pick up where you left off?") == true) {
      ...
      var totalExpedites = user.model.reward;
      var expeditesInQuote = $(document).find('.expedited:checked').length;
      if ( (totalExpedites - expeditesInQuote)  > 0 ) {
        $("#reward-counter").val((totalExpedites - expeditesInQuote));
        $("#reward-box").removeClass('hide-price');
      } else {
...
var additive = $("#color-2").val();
  ...
  var expedite = `style="display: block;" checked`;
  } else {
  var expedite = `style="display:none;"`;  
  }
...
<div class="row">
    ...
    <input type="checkbox" class="expedited" id="l`+lineItem+`-expedite" readonly `+ expedite +`>Expedited</input>
	</div>
	<div class="col">
