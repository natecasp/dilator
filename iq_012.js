...
if(od - id < 0.070) {
  od = Number(id) + 0.070
  odController.setValue(Number(id)+0.070)
}
...
let m = new THREE.MeshStandardMaterial({color: "#8ac1c3", depthWrite: false, roughness: .1,transparent: true,opacity: .5});
...
$("#5-week-price").on('change', function(){
...
function checkTaper(taper, minAngle) {
   if (Number(taper) < minAngle) { $("#taper").addClass("error"); } else { $("#taper").removeClass("error");}
}
...
if (unit == 'in') {
//VALIDATE
function validate(wallMin, wallMax, minAngle, wallTol, isSoft){
  ...
  var wallTolerance = $("#wall-tol").val();
  var length = Number($("#length-2").val());
  if((wall >= wallMin && wall <= wallMax) && taper >= minAngle && (length <=60) && (length >=12) && (qty <= 50) && ( od <= 0.350 ) && ( id >= 0.02 ) && (lengthTol == 'min') && (material !== '') && (Number(odTolerance) >= Number(odTol)) && (Number(wallTolerance) >= Number(wallTol))) { 
    $("#custom-quote").prop('checked', false);
    console.log('not-greenlight');
    $("#custom-quote").trigger('change');
    greenlight.prop('checked', true);
    greenlight.trigger('change'); 
    $("#wall,#length-2,#length-tol-2,#od-tol-2,#wall-tol,#id-2,#quantity-2").removeClass("error");
    ...
    //Check OD
    if ( od > 0.300 ) { $("#od-3").addClass("error"); } 
...
if (['Pebax 45D','Pebax 55D','Pebax 63D', 'Pebax 70D', 'Pebax 72D', 'Pebax 74D', 'Vestamid ML21','PET','Delrin'].includes($("#material-2").val()) ){
  ...
  var wallTol = 0.002;
  var minAngle = 3;
  validate(wallMin, wallMax, minAngle, wallTol, false);
  }
