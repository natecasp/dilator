...
function validate(wallMin, wallMax, minAngle, wallTol, odTol, isSoft){
  ...
  var wallTolerance = $("#wall-tol").val();
  var length = Number($("#length-2").val());
  if((wall >= wallMin && wall <= wallMax) && (taper >= minAngle) && (taperTolerance >= 1) && (length <=60) && (length >=12) && (qty <= 50) && ( od <= 0.350 ) && ( id >= 0.02 ) && (lengthTol == 'min') && (material !== '') && (Number(odTolerance) >= Number(odTol)) && (Number(wallTolerance) >= Number(wallTol))) { 
    $("#custom-quote").prop('checked', false);
    console.log('not-greenlight');
    $("#custom-quote").trigger('change');
    greenlight.prop('checked', true);
    greenlight.trigger('change'); 
    $("#wall,#length-2,#length-tol-2,#od-tol-2,#wall-tol,#id-2,#quantity-2, #taper").removeClass("error");
