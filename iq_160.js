...
const tipController = folder.add(params, 'tip_ID', {0.014:0.014, 0.018:0.018, 0.022:0.022, 0.025:0.025, 0.035:0.035}).onChange(createShape);
...
$("#4-week-price").val(((Number(startPrice)+6469)*discount).toFixed(2));
$("#4-week-discount").text('$' + (Number(startPrice)+6469).toFixed(2));
$("#6-week-price").val(((Number(startPrice)+4740)*discount).toFixed(2));
$("#6-week-discount").text('$' + (Number(startPrice)+4740).toFixed(2));
...
function preventImpossible(converting) {
  ...
  if ( tipIdLen > taperlen ) {
    $("#tip-id-len, #tip-id-len-range").val(taperlen - tol).trigger("change");
    return false;
  }
  // Prevent ID from being smaller than tip ID
  if (id <= tipId) {
    $("#inner-diameter, #id-range").val(tipId);
    idController.setValue(tipId);
    return false;
  }
...
function checkIdLength (id, tipIdLen) {
  ...
  if( angle < 1.047 ) {
    addError(
      'id-angle',
      'The Guide Wire Collar is too long to qualify for an instant quote.',
      'Decrease Guide Wire Collar, or continue with a custom quote.',
      null,
      '#tip-id-len'
    );
    return false;
...
$("#material-2").on("change", function() {
   ...
   if (['Pebax 25D','Pebax 35D','Pebax 45D','Pebax 55D','Pebax 63D','Pebax 72D','Vestamid ML21'].includes(material)) { $("#colorant").empty().append('<option value="">Select a Color...</option><option value="White">White</option><option value="Black">Black</option><option value="Cool Grey (7C)">Cool Grey (7C)</option><option value="Green (3248C)">Green (3248C)</option><option value="Green (3268C)">Green (3268C)</option><option value="Green (3298C)">Green (3298C)</option><option value="Blue (290C)">Blue (290C)</option><option value="Blue (295C)">Blue (295C)</option><option value="Blue (2925C)">Blue (2925C)</option><option value="Blue (301C)">Blue (301C)</option><option value="Purple (2655C)">Purple (2655C)</option>'); return }
   if (['LDPE'].includes(material)) {$("#colorant").empty().append('<option value="None">None</option><option value="Cool Grey (4C)">Cool Grey (4C)</option>'); $("#color-2").empty().append('<option value="None">None</option><option value="BaSO4">20% BaSO4</option>');  return;}
   if (['HDPE'].includes(material)) {$("#color-2").empty().append('<option value="None">None</option>'); $("#colorant").empty().append('<option value="None">None</option><option value="Cool Grey (4C)">Cool Grey (4C)</option>'); return;}
   if (['Pebax 25D','Pebax 35D','Pebax 55D', 'Pebax 63D', 'Pebax 72D', 'Pebax 45D'].includes(material)) { $("#color-2").empty().append('<option value="None">None</option><option value="BaSO4">20% BaSO4</option><option value="Lubricious Additive">Lubricious Additive</option><option value="BaSO4 & Lubricious Additive">20% BaSO4 & Lubricious Additive</option>'); return;} 
...
function calculate() {
  ...
  if (['Pebax 63D','Pebax 55D','HDPE', 'LDPE'].includes(material)) {
    if (Number(qty) == 10) {
      $("#8-week-price").val(5660);
    } else if (Number(qty) == 25) {
      $("#8-week-price").val(6410);
    } else if (Number(qty) == 50) {
      $("#8-week-price").val(7660);
    }
  }
...
<div class="row">
    ...
    <div class="item"><p class="label">GW Collar</p><input class="quote-input" id="l`+lineItem+`-tip-id-len" value="`+tipIdLen+`" readonly></input></div>
...
$("#tip-id-len, #tip-id-len-range").val($("#l"+ line + "-tip-id-len").val());
...
$("#l"+ line + "-tip-id-len").val($("#tip-id-len").val());  
