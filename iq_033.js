...
let outer = new THREE.CylinderGeometry( od, od, 1, 64, 15, true );
let inner = new THREE.CylinderGeometry( id, id, 1, 64, 15, true );
...
oTap.translateY(.5 + (taperLength/2)-radius/2);
iTap.translateY(.5+ (taperLength/2));
tipRadius.translateY(.5+ (taperLength - radius));
...
$("#5-week-price").val(((Number(startPrice)+1100)*discount).toFixed(2));
$("#5-week-discount").text('$' + (Number(startPrice)+1100).toFixed(2));
$("#6-week-price").val(((Number(startPrice)+600)*discount).toFixed(2));
$("#6-week-discount").text('$' + (Number(startPrice)+600).toFixed(2));
...
function validate(wallMin, wallMax, minAngle, wallTol, odTol, isSoft){
  ...
  var wallTolerance = $("#wall-tol").val();
  var length = Number($("#length-2").val());
  if((wall >= wallMin && wall <= wallMax) && (taper >= minAngle) && (taperTolerance >= 1) && (length <=60) && (length >=12) && (qty <= 50) && ( od <= 0.350 ) && ( od >= 0.02 ) && (lengthTol == 'min') && (material !== '') && (tipId !== '') && (Number(odTolerance) >= Number(odTol)) && (Number(wallTolerance) >= Number(wallTol))) { 
    ...
    else if ( od < 0.014 ) { $("#od-3").addClass("error"); } else {$("#od-3").removeClass("error");}
...
if (['Pebax 45D','Pebax 55D','Pebax 63D', 'Pebax 70D', 'Pebax 72D', 'Pebax 74D', 'Vestamid ML21','PET','Delrin'].includes($("#material-2").val()) ){
  ...
  var wallMin = (Number(od) * 0.4) / 2;
  var wallMax = (Number(od) * 0.6) / 2;
  var wallTol = 0.002;
  var odTol = 0.002;
  var minAngle = 3;
...
idController.setValue((Number(this.value) - (wall * 2)).toFixed(sig));
var tol = ((Number(this.value) * 0.25) / 2);
$("#wall, #wall-range").attr('min', tol.toFixed(wsig)); 
  
$("#id-2").val((Number(this.value) - (wall * 2)).toFixed(sig));
  calculate(false);
$("wall, #wall-range").attr("max", (Number(this.value)-(Number($("#tip-id").val())))/2);
var minW = (Number(this.value) * 0.4) /2;
var maxW = (Number(this.value) * 0.6) /2;
var max = Number($("#wall-range").attr('max'));
var maxP = (maxW / max)*100;
if ($("#wall-possibility")) {
$("#wall-possibility").remove();
console.log('min wall: ' + minW + 'max wall: ' + maxW + 'rangeMax: '+ max + 'left: ' + ((minW / max)*100));
}
...
var minW = (Number(this.value) * 0.4) /2;
var maxW = (Number(this.value) * 0.6) /2;
var max = $("#wall-range").attr('max');
var maxP = (maxW / max)*100;
if ($("#wall-possibility")) {
$("#wall-possibility").remove();
}
$("#wall-slider-container").append(`<div id="wall-possibility" style="position:absolute;border: 2px solid #8ac1c3; border-bottom:none;border-radius: 5px 5px 0px 0px; top:-5px;height:7px;width:`+ (((maxW / Number(max))*100) - ((minW / Number(max))*100)) +`%;left:`+ ((minW / Number(max))*100) +`%;"></div>`);
...
// if($(":radio[value=id]").is(":checked")) {
   ...
   $("wall, #wall-range").attr("max", (Number($("#od-3").val())-(Number($("#tip-id").val())))/2);
...
$("#tip-id").empty().append(`<option value='0.016'>0.014" GW Compatible [0.016" True]</option><option value='0.020'>0.018" GW Compatible [0.020" True]</option><option value='0.024'>0.022" GW Compatible [0.024" True]</option><option value='0.037'>0.035" GW Compatible [0.037" True]</option>`);
...
$("#radius, #radius-range").val((originalRadius / 25.4).toFixed(3));
...
$("#tip-id").empty().append(`<option value='0.406'>0.014" GW Compatible [0.406mm True]</option><option value='0.508'>0.018" GW Compatible [0.508mm True]</option><option value='0.610'>0.022" GW Compatible [0.610mm True]</option><option value='0.940'>0.035" GW Compatible [0.940mm True]</option>`);
...
$("#l"+ line + "-taper-tol").val($("#taperTol").val());
$("#l"+ line + "-tip-id").val($("#tipId").val());
